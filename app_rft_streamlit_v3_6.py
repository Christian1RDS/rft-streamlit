...
st.set_page_config(page_title="RFT Automático — V3.6", page_icon="📈", layout="wide")
...
def is_working_day(c: sqlite3.Connection, d: date) -> bool:
    ...
    if ov is not None:
        return ov
    return d.weekday() not in (5, 6)  # sáb/dom não trabalhados por padrão
...
def render_help():
    with st.expander('O que mudou na V3.6', expanded=False):
        st.markdown(
            """
...
def main():
    c = get_conn()
    init_db(c)
    st.markdown("<div class='hero'><h1>RFT Automático — V3.6</h1><p>Versão estável com correção do erro de sintaxe, meta visual corrigida, calendário funcional e processamento completo por dia/semana/mês/ano/YTD.</p></div>", unsafe_allow_html=True)
    sidebar_controls(c)
    render_dashboard(c)
    st.divider()
    render_imports(c)
