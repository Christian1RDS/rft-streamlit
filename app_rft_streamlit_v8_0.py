...
def resolve_period_selection(df, ano, mode):
    ...
    if mode == 'Diario':
        ...
        if default_day < min_date or default_day > max_date:
            default_day = max_date
        selected = st.sidebar.date_input('Dia', value=default_day, min_value=min_date, max_value=max_date, format='DD/MM/YYYY', key='day_v80')
...
def main():
    ...
    with tabs[0]:
        if year_df.empty:
            ...
            _, max_date, selected_label, selected_range, valid = resolve_period_selection(year_df, ano, mode)
            if not valid:
