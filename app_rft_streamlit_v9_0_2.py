...
st.set_page_config(page_title='RFT Automatico - V9.0.2', page_icon='R', layout='wide', initial_sidebar_state='expanded')
...
def resolve_period_selection(df, ano, mode):
    ...
    if mode == 'Diario':
        ...
        if default_day < min_date or default_day > max_date:
            default_day = max_date
        selected = st.sidebar.date_input('Dia', value=default_day, min_value=min_date, max_value=max_date, format='DD/MM/YYYY', key='day_v902')
        ...
        label = st.sidebar.selectbox('Semana do ano', labels, index=idx, key='week_v902')
        ...
        idx = labels.index(saved) if saved in labels else len(labels) - 1
        label = st.sidebar.selectbox('Mes do ano', labels, index=idx, key='month_v902')
...
def render_meta_resultado(metrics, meta):
    ...
    color = 'normal' if current_pct is None or current_pct <= meta else 'inverse'
    val = 'Sem dados' if current_pct is None else f"{current_pct:.2f}".replace('.', ',') + '%'
    st.subheader('Meta x resultado')
    st.caption('Regra visual aplicada conforme sua configuracao.')
    st.metric('Resultado atual', val, diff, delta_color=color)
    st.write(f"Meta: **{str(meta).replace('.', ',')}%**")
    st.write('Regra: RFT <= meta fica verde | RFT > meta fica vermelho')


def render_resumo(info, posto, ano, min_date, max_date, selected_label, meta):
    st.subheader('Resumo executivo do recorte')
    st.write(f"Recorte: **{selected_label}**")
    st.write(f"Ultimo arquivo salvo: **{info['file_name'] if info else '-'}**")
    st.write(f"Ultimo upload: **{info['uploaded_at'] if info else '-'}**")
    st.write(f"Posto: **{posto}**")
    st.write(f"Ano: **{ano}**")
    st.write(f"Janela consolidada: **{min_date.strftime('%d/%m/%Y')} ate {max_date.strftime('%d/%m/%Y')}**")
    st.write(f"Meta ativa: **{str(meta).replace('.', ',')}%**")


def main():
    conn = get_conn()
    init_db(conn)
    st.title('RFT Automatico - V9.0.2')
    st.caption('Versão corretiva com calendário, YTD, upload corrigido e novos blocos no dashboard.')
    ...
    with tabs[0]:
        if ano is None or df.empty:
            ...
            if not valid:
                ...
                hist_diario = daily_history_df(df)
                st.subheader('Leitura diaria do RFT')
                st.caption('Historico dia a dia dentro da base consolidada.')
                if hist_diario.empty:
                    st.info('Sem dados diários disponíveis.')
                else:
                    st.line_chart(hist_diario.set_index('Dia'), use_container_width=True)
    ...
    with tabs[2]:
        ...
        import_mode = st.radio('Modo de importação', ['Somar ao historico', 'Substituir periodo sobreposto', 'Reprocessar o ano inteiro'], key='import_mode_v902')
        uploaded = st.file_uploader('Base operacional atual (.xlsx, .xls ou .csv)', type=['xlsx', 'xls', 'csv'], key='file_uploader_v902')
        prepared = None
        ...
        if st.button('Salvar arquivo localmente', type='primary', use_container_width=True, key='save_upload_v902'):
            if uploaded is None:
    ...
    with tabs[3]:
        ...
        if hist.empty:
            ...
            selected_id = st.selectbox('Selecionar upload', hist['id'].tolist(), format_func=lambda x: f'Upload {x}', key='history_select_v902')
            c1, c2, c3 = st.columns(3)
            with c1:
                if st.button('Ver detalhes do upload', use_container_width=True, key='history_view_v902'):
            ...
            with c2:
                if st.button('Reprocessar upload', use_container_width=True, key='history_reprocess_v902'):
                    ...
                    st.rerun()
            with c3:
                if st.button('Excluir upload específico', use_container_width=True, key='history_delete_v902'):
    ...
    with tabs[4]:
        st.subheader('Sobre')
        st.write('Versão V9.0.2: mantém calendário, modos Diário / Semanal / Mensal / Anual, YTD, gráfico mensal em barras e adiciona Meta x resultado, Resumo executivo do recorte e Leitura diária do RFT no Dashboard.')
