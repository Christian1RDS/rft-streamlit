...
st.set_page_config(page_title="RFT Automático — V3.5", page_icon="📈", layout="wide")
DB = "rft_v35.db"
...
def get_override(c, d: date):
    row = c.execute('SELECT is_working FROM work_calendar WHERE work_date=?', (d.isoformat(),)).fetchone()
    return None if row is None else bool(row[0])

def is_working_day(c, d: date):
    ov = get_override(c, d)
    if ov is not None:
        return ov
    return d.weekday() not in (5,6)

def save_calendar(c, d: date, is_working: bool, note: str=''):
    c.execute("INSERT INTO work_calendar (work_date, is_working, note, updated_at) VALUES (?, ?, ?, ?) ON CONFLICT(work_date) DO UPDATE SET is_working=excluded.is_working, note=excluded.note, updated_at=excluded.updated_at", (d.isoformat(), 1 if is_working else 0, note, datetime.now().isoformat(timespec='seconds')))
    c.commit()

def prev_working_day(c, ref: date):
...
def resolve_daily_effective(c, ref: date):
    if ref.weekday() == 0:
...
def calc_rft(df, start_date: date, end_date: date):
...
def update_upload(c, upload_id, status, message=''):
    c.execute('UPDATE upload_log SET status=?, message=? WHERE id=?', (status, message, upload_id)); c.commit()
...
def latest_upload(c, role='current'):
    ...
    return c.execute('SELECT * FROM upload_log WHERE status="PROCESSADO" AND dataset_role=? ORDER BY id DESC LIMIT 1', (role,)).fetchone()

def hist_upload_for_year(c, year_tag):
    c.row_factory = sqlite3.Row
    return c.execute('SELECT * FROM upload_log WHERE status="PROCESSADO" AND dataset_role="historical" AND year_tag=? ORDER BY id DESC LIMIT 1', (year_tag,)).fetchone()
...
def delete_metrics_for_upload(c, upload_id):
...
def generate_metrics(c, upload_id, df):
    delete_metrics_for_upload(c, upload_id)
    if df.empty:
        return
    ...
    for y,m in sorted({(d.year,d.month) for d in unique_dates}):
        start = date(y,m,1); end = date(y,m,monthrange(y,m)[1]); r=calc_rft(df,start,end)
...
def summary_export(c, current, comparison_year):
    ...
    with pd.ExcelWriter(out, engine='openpyxl') as writer:
        pd.DataFrame([['Arquivo operacional', current['file_name']], ['Último upload', current['uploaded_at']], ['Ano de comparação', comparison_year]], columns=['Campo','Valor']).to_excel(writer, index=False, sheet_name='Resumo')
...
def render_dashboard(c):
    ...
    note_year = f'Usando base histórica dedicada de {comp_year}.' if h else (f'Usando a base operacional para {comp_year}.' if year_metric else f'Sem dados de {comp_year}; envie uma base histórica opcional.')
    ...
    if daily_v is None:
        st.markdown(alert_box_html('Status diário', 'Sem dados para a data de referência selecionada.', 'info'), unsafe_allow_html=True)
    elif daily_v <= target:
        st.markdown(alert_box_html('Status diário', f'RFT diário dentro da meta. Meta = {format_pct(target)} | Atual = {format_pct(daily_v)}.', 'ok'), unsafe_allow_html=True)
    else:
        st.markdown(alert_box_html('Alerta diário', f'RFT diário acima da meta. Meta = {format_pct(target)} | Atual = {format_pct(daily_v)}.', 'crit'), unsafe_allow_html=True)
    if weekly_v is not None and weekly_v > target:
        st.markdown(alert_box_html('Alerta semanal S-1', f'RFT semanal acima da meta. Atual = {format_pct(weekly_v)}.', 'crit'), unsafe_allow_html=True)
    if monthly_v is not None and monthly_v > target:
        st.markdown(alert_box_html('Alerta mensal', f'RFT mensal acima da meta. Atual = {format_pct(monthly_v)}.', 'crit'), unsafe_allow_html=True)
    ...
    cols2 = st.columns(4)
    cols2[0].markdown(card(f'RFT {comp_year}', format_pct(year_metric['rft_pct'] if year_metric else None), note_year), unsafe_allow_html=True)
    ...
    st.markdown(f"<span class='chip'><strong>Arquivo operacional:</strong> {cur['file_name']}</span><span class='chip'><strong>Último upload:</strong> {cur['uploaded_at']}</span><span class='chip'><strong>Data de referência:</strong> {format_date(ref)}</span><span class='chip'><strong>Regra diária:</strong> {note}</span><span class='chip'><strong>Ano de comparação:</strong> {comp_year}</span><span class='chip'><strong>Meta:</strong> {format_pct(target)}</span>", unsafe_allow_html=True)
...
def render_imports(c):
    ...
    current_file = st.file_uploader('Base operacional atual (.xlsx, .xls ou .csv)', type=['xlsx','xls','csv'], key='current_import')
    hist_file = st.file_uploader(f'Base histórica opcional para {cy} (.xlsx, .xls ou .csv)', type=['xlsx','xls','csv'], key='hist_import')
    ...
    if c1.button('🔄 Processar bases e recalcular todos os períodos', type='primary', use_container_width=True):
        ...
        if hist_file is not None:
            try:
                ...
                ok2, miss2 = validate(h_raw)
                if not ok2:
                    st.error('Base histórica inválida: ' + ', '.join(miss2)); st.stop()
                h_df = prepare(h_raw)
            except Exception as e:
                st.error(f'Erro ao ler a base histórica: {e}'); st.stop()
            h_id = create_upload(c, hist_file.name, len(h_df), role='historical', year_tag=cy, message=f'Base histórica {cy} recebida.')
        try:
            save_raw(c, cur_id, cur_df); generate_metrics(c, cur_id, cur_df); update_upload(c, cur_id, 'PROCESSADO', 'Base operacional processada com sucesso.')
            if h_df is not None and h_id is not None:
                save_raw(c, h_id, h_df); generate_metrics(c, h_id, h_df); update_upload(c, h_id, 'PROCESSADO', f'Base histórica {cy} processada com sucesso.')
            ...
            if h_id is not None: update_upload(c, h_id, 'ERRO', str(e))
...
def render_help():
    with st.expander('O que mudou na V3.5', expanded=False):
        st.markdown('- Meta visual corrigida: **menor ou igual à meta = verde** e **acima da meta = vermelho**.
...
def main():
    c = conn(); init_db(c)
    st.markdown("<div class='hero'><h1>RFT Automático — V3.5</h1><p>Meta corrigida (menor é melhor), calendário funcional, processamento completo por dia/semana/mês/ano/YTD e base histórica separada para ano de comparação.</p></div>", unsafe_allow_html=True)
    sidebar_controls(c)
    render_dashboard(c)
    st.divider(); render_imports(c)
    st.divider(); render_metric_tabs(c)
    st.divider(); render_upload_history(c)
