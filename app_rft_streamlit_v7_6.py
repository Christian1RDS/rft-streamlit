...
def card(title, result, subtitle=''):
    if result is None:
        st.metric(title, 'Sem dados', subtitle)
    else:
        st.metric(title, format_pct(result['rft_pct']), f"{subtitle} | Bons: {result['good']} | Ruins: {result['bad']} | Total: {result['total']}")


def main():
    conn = get_conn()
    init_db(conn)
    st.title('Qualidade | RFT Automatico - V7.6')
    st.caption('Versão em arquivo único com RFT completo + Rolling 12 separado.')
    ...
    with tabs[0]:
        if year_df.empty:
            ...
            if not valid:
                ...
                cols = st.columns(5)
                with cols[0]: card('Diario', cards['daily'], 'Dia selecionado')
                with cols[1]: card('Semanal', cards['weekly'], 'Semana')
                with cols[2]: card('Mensal', cards['monthly'], 'Mês')
                with cols[3]: card('Anual', cards['yearly'], 'Ano até 12/12')
                with cols[4]: card('YTD', cards['ytd'], 'Ano até data')
                st.caption(f"Recorte: {selected_label} | Fechamento: {status_label} | Arquivo: {info['file_name'] if info else '-'}")
    ...
    with tabs[2]:
        if all_df.empty or year_df.empty:
            ...
            c1, c2 = st.columns(2)
            with c1: card('Rolling 12 Consolidado', rolling12, '12 meses móveis')
            with c2: card('YTD atual', calc_rft(year_df, date(ano,1,1), year_df['DT_HR_INSPECAO'].dt.date.max()), 'Ano até data')
    ...
    with tabs[5]:
        st.write('Versão V7.6 em arquivo único, criada para evitar dependências entre arquivos e corrigir o erro de NameError da V7.5. Mantém calendário, Tendências, YTD e Rolling 12 separado.')
