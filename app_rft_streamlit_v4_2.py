...
st.set_page_config(page_title="RFT Automático — V4.2", page_icon="📊", layout="wide")
...
def render_dashboard(c):
    ...
    if df.empty:
        st.warning(f'Não há dados de {posto} no arquivo carregado. O app não mistura {POSTOS[0]} com {POSTOS[1]}.')
        return
    ...
    if mode == 'Diário':
        selected = st.date_input('Clique no dia que você quer ver', value=max_date, min_value=min_date, max_value=max_date, format='DD/MM/YYYY', key='daily_picker_v42')
        ...
        complete, note = is_full_year_available(df, selected.year)
        yearly = calc_rft(df, date(selected.year, 1, 1), date(selected.year, 12, 31)) if complete else {'rft_pct': None, 'total': 0, 'good': 0, 'bad': 0, 'note': note}
        ytd = calc_rft(df, date(selected.year, 1, 1), selected)
        context = f'Dia selecionado: {format_date(selected)}'

    elif mode == 'Semanal':
        selected = st.date_input('Escolha qualquer dia da semana que você quer ver', value=max_date, min_value=min_date, max_value=max_date, format='DD/MM/YYYY', key='weekly_picker_v42')
        week_start = selected - timedelta(days=selected.weekday())
        week_end = week_start + timedelta(days=6)
        weekly = calc_rft(df, week_start, week_end)
        ytd = calc_rft(df, date(selected.year, 1, 1), selected)
        ...
        selected = st.date_input('Escolha qualquer dia do mês que você quer ver', value=max_date, min_value=min_date, max_value=max_date, format='DD/MM/YYYY', key='monthly_picker_v42')
        month_start = date(selected.year, selected.month, 1)
        month_end = date(selected.year, selected.month, monthrange(selected.year, selected.month)[1])
        monthly = calc_rft(df, month_start, month_end)
        ytd = calc_rft(df, date(selected.year, 1, 1), selected)
        ...
        complete, note = is_full_year_available(df, selected_year)
        if complete:
            yearly = calc_rft(df, date(selected_year, 1, 1), date(selected_year, 12, 31))
        else:
        ...
        if pd.notna(last_day):
            ytd = calc_rft(df, date(selected_year, 1, 1), last_day)
...
def render_tables(c):
    ...
    if cur is None:
        ...
        return
    df_all = load_upload_df(c, cur['id'])
    posto = st.radio('Posto das tabelas', POSTOS, index=0, horizontal=True, key='table_posto_v42')
    df = df_all[df_all['CD_POSTO_CN'] == posto].copy()
    if df.empty:
...
- Depois disso, você escolhe se quer ver **Diário, Semanal, Mensal ou Anual**.
...
def main():
    c = get_conn()
    init_db(c)
    st.markdown("<div class='hero'><h1>RFT Automático — V4.2</h1><p>Versão ajustada para usar apenas QG09 e QG07, com QG09 como padrão e opção de alternar para QG07 sem misturar os resultados.</p></div>", unsafe_allow_html=True)
    render_dashboard(c)
    st.divider()
    render_imports(c)
    st.divider()
