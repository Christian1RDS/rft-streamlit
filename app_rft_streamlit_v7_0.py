...
<style>
:root {
  --bg:#0f172a; --panel:#182235; --card:#1f2a3d; --border:rgba(148,163,184,.18); --text:#e5e7eb; --muted:#94a3b8; --green:#22c55e; --red:#ef4444; --amber:#f59e0b; --blue:#3b82f6;
}
...
.hero-grid { display:grid; grid-template-columns:1.6fr 1fr; gap:1rem; align-items:center; }
.hero-brand { display:flex; gap:1rem; align-items:center; }
.hero-title { font-size:1.75rem; font-weight:800; margin-bottom:.1rem; }
.hero-sub { color:var(--muted); font-size:.95rem; }
.hero-pills { display:flex; flex-wrap:wrap; gap:.5rem; justify-content:flex-end; }
.pill { display:inline-flex; align-items:center; gap:.35rem; padding:.45rem .75rem; border-radius:999px; border:1px solid var(--border); background: rgba(255,255,255,.04); font-size:.84rem; }
.pill-blue{background:rgba(59,130,246,.12);border-color:rgba(59,130,246,.28);} .pill-green{background:rgba(34,197,94,.12);border-color:rgba(34,197,94,.28);} .pill-red{background:rgba(239,68,68,.12);border-color:rgba(239,68,68,.28);} .pill-amber{background:rgba(245,158,11,.12);border-color:rgba(245,158,11,.28);} 
...
.section-title { font-size:1.06rem; font-weight:700; margin-bottom:.2rem; }
.section-sub { color:var(--muted); font-size:.9rem; margin-bottom:.8rem; }
.metric-card { border:1px solid var(--border); background:linear-gradient(180deg,rgba(31,42,61,.96),rgba(22,31,47,.98)); border-radius:18px; padding:1rem; min-height:152px; }
.metric-title { color:var(--muted); font-size:.88rem; margin-bottom:.45rem; } .metric-value { font-size:1.9rem; font-weight:800; margin-bottom:.35rem; } .metric-meta { color:var(--muted); font-size:.82rem; line-height:1.35; }
.ok{color:var(--green);} .bad{color:var(--red);} .neutral{color:var(--text);} .warn{color:var(--amber);} 
.info-box { border:1px dashed rgba(59,130,246,.35); background:rgba(59,130,246,.08); border-radius:16px; padding:.9rem 1rem; }
.status-chip { display:inline-flex; align-items:center; gap:.35rem; padding:.35rem .65rem; border-radius:999px; font-size:.78rem; font-weight:700; }
.status-melhorou{color:#d1fae5;background:rgba(34,197,94,.14);} .status-piorou{color:#fecaca;background:rgba(239,68,68,.14);} .status-estavel{color:#fef3c7;background:rgba(245,158,11,.14);} 
...
def week_options(df, year):
    ...
    return [(f'Semana {i:02d} - {monday.strftime("%d/%m/%Y")} a {(monday+timedelta(days=6)).strftime("%d/%m/%Y")}', monday, monday + timedelta(days=6)) for i, monday in enumerate(mondays, start=1)]
...
def compute_selected_metrics(df, ano, mode, selected_range, ok_ano, max_date):
    ...
    if mode == 'Diario':
        sel = start
        daily = calc_rft(df, sel, sel)
        ws = sel - timedelta(days=sel.weekday())
        weekly = calc_rft(df, ws, ws + timedelta(days=6))
        ms = date(ano, sel.month, 1)
        monthly = calc_rft(df, ms, date(ano, sel.month, monthrange(ano, sel.month)[1]))
...
def metric_card(title, result, subtitle=''):
    if result is None:
        value = 'Sem dados'; css = 'neutral'; meta_text = subtitle
    else:
...
def render_hero(info, posto, ano, status_label, base_summary, meta):
    ...
    st.markdown(f'''<div class="hero"><div class="hero-grid"><div class="hero-brand"><div class="hero-logo">Q</div><div><div class="hero-title">Qualidade | RFT Automatico - V7.0</div><div class="hero-sub">Painel corporativo com consolidacao inteligente de uploads, meta configuravel e leitura executiva da base.</div></div></div><div class="hero-pills"><div class="pill pill-blue">Posto: <strong>{posto}</strong></div><div class="pill {base_summary['status_cls']}">Status da base: <strong>{base_summary['status']}</strong></div><div class="pill pill-blue">Meta: <strong>{str(meta).replace('.', ',')}%</strong></div><div class="pill pill-blue">Uploads consolidados: <strong>{base_summary['uploads_count']}</strong></div></div></div><div style="height:10px"></div><div style="color:#94a3b8;">Ano: <b>{ano}</b> | Fechamento: <b>{status_label}</b> | Ultimo arquivo: <b>{file_name}</b> | Ultimo upload: <b>{uploaded_at}</b> | Janela consolidada: <b>{format_date(base_summary['min_date'])} ate {format_date(base_summary['max_date'])}</b></div></div>''', unsafe_allow_html=True)
...
def render_dashboard(df, metrics, selected_label, info, posto, ano, status_label, meta, base_summary):
    ...
    with left:
        ...
        for label, value in [
            ...
            ('Data minima da base', format_date(base_summary['min_date'])),
            ('Data maxima da base', format_date(base_summary['max_date'])),
            ('Ultima data com dado', format_date(base_summary['last_date'])),
    ...
    with right:
        ...
        st.markdown(f'<div class="section"><div class="section-title">Meta x resultado</div><div class="section-sub">Regra visual aplicada conforme sua configuracao.</div><div class="metric-value {color}">{format_pct(current_pct)}</div><div style="color:#94a3b8;">Meta configurada: {str(meta).replace(".", ",")}%<br>Diferenca: {diff}<br>Regra: RFT <= meta fica verde | RFT > meta fica vermelho</div></div>', unsafe_allow_html=True)


def render_monthly_chart(monthly_df, meta):
    fig, ax = plt.subplots(figsize=(9, 3.8))
    colors = ['#22c55e' if val <= meta else '#ef4444' for val in monthly_df['RFT']]
    ax.bar(monthly_df['Mes'], monthly_df['RFT'], color=colors, alpha=0.85, label='RFT')
    ax.plot(monthly_df['Mes'], monthly_df['Meta'], color='#f59e0b', linewidth=2.2, linestyle='--', label='Meta')
    ax.plot(monthly_df['Mes'], monthly_df['Acumulado_Ano'], color='#60a5fa', linewidth=2.0, marker='o', label='Acumulado ano')
    best_idx, worst_idx = monthly_df['RFT'].idxmax(), monthly_df['RFT'].idxmin()
    ax.scatter(monthly_df.loc[best_idx, 'Mes'], monthly_df.loc[best_idx, 'RFT'], s=90, color='#22c55e', edgecolors='white', zorder=5)
    ax.scatter(monthly_df.loc[worst_idx, 'Mes'], monthly_df.loc[worst_idx, 'RFT'], s=90, color='#ef4444', edgecolors='white', zorder=5)
    ax.annotate('Melhor', (monthly_df.loc[best_idx, 'Mes'], monthly_df.loc[best_idx, 'RFT']), textcoords='offset points', xytext=(0, 9), ha='center', color='white')
    ax.annotate('Pior', (monthly_df.loc[worst_idx, 'Mes'], monthly_df.loc[worst_idx, 'RFT']), textcoords='offset points', xytext=(0, -14), ha='center', color='white')
    ax.set_ylim(0, max(100, float(monthly_df[['RFT', 'Meta', 'Acumulado_Ano']].max().max()) + 5))
    ax.set_ylabel('RFT (%)')
    ax.set_xlabel('Mes')
    ax.set_title('Tendencia mensal do RFT')
...
def render_tendencia(df, ano, meta):
    ...
    st.markdown('<div class="section"><div class="section-title">Tendencia mensal</div><div class="section-sub">Linha de meta destacada, grafico por performance, marcador de melhor e pior periodo, comparacao entre meses e acumulado do ano.</div>', unsafe_allow_html=True)
    if monthly_df.empty:
        st.info('Sem dados mensais disponiveis.')
    else:
        render_monthly_chart(monthly_df, meta)
        latest = monthly_df.iloc[-1]
        prev = monthly_df.iloc[-2] if len(monthly_df) > 1 else None
...
def render_about(meta):
    st.markdown(f'<div class="section"><div class="section-title">Sobre a V7.0</div><div class="section-sub">Versao com foco em governanca da base e analise gerencial da Qualidade.</div><div class="info-box">Inclui modos de importacao, aviso de sobreposicao, status da base, quantidade de uploads consolidados, acoes de historico, linha de meta mais visivel, marcadores de melhor e pior periodo, comparacao entre mes anterior e atual, tendencia acumulada no ano e status visual de melhora/piora/estabilidade. Regra visual ativa: valores iguais ou menores que <b>{str(meta).replace(".", ",")}%</b> ficam verdes; valores acima da meta ficam vermelhos.</div></div>', unsafe_allow_html=True)
