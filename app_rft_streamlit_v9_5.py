...
st.set_page_config(page_title='RFT Automatico - V9.5', page_icon='R', layout='wide', initial_sidebar_state='expanded')
...
def resolve_period_selection(df, ano, mode):
    ...
    if mode == 'Diario':
        ...
        selected = st.sidebar.date_input('Dia', value=default_day, min_value=min_date, max_value=max_date, format='DD/MM/YYYY', key='day_v95')
        ...
        label = st.sidebar.selectbox('Semana do ano', labels, index=idx, key='week_v95')
        ...
        label = st.sidebar.selectbox('Mes do ano', labels, index=idx, key='month_v95')
...
def main():
    ...
    st.markdown('<div class="hero"><div style="font-size:1.75rem;font-weight:900;">RFT Automatico - V9.5</div><div class="muted">Design corporativo refinado, mantendo calendário, YTD, upload e tendência mensal em barras.</div></div>', unsafe_allow_html=True)
    with st.sidebar:
    ...
    with tabs[2]:
        ...
        import_mode = st.radio('Modo de importação', ['Somar ao historico','Substituir periodo sobreposto','Reprocessar o ano inteiro'], key='import_mode_v95')
        uploaded = st.file_uploader('Base operacional atual (.xlsx, .xls ou .csv)', type=['xlsx','xls','csv'], key='file_uploader_v95')
        prepared = None
        ...
        if st.button('Salvar arquivo localmente', type='primary', use_container_width=True, key='save_upload_v95'):
            if uploaded is None:
    ...
    with tabs[3]:
        ...
        if hist.empty:
            ...
            selected_id = st.selectbox('Selecionar upload', hist['id'].tolist(), format_func=lambda x: f'Upload {x}', key='history_select_v95')
            c1,c2,c3 = st.columns(3)
            with c1:
                if st.button('Ver detalhes do upload', use_container_width=True, key='history_view_v95'):
            ...
            with c2:
                if st.button('Reprocessar upload', use_container_width=True, key='history_reprocess_v95'):
                    ...
                    st.rerun()
            with c3:
                if st.button('Excluir upload específico', use_container_width=True, key='history_delete_v95'):
                    delete_upload(conn, selected_id)
                    st.success('Upload excluído com sucesso.')
                    st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    with tabs[4]:
        st.markdown('<div class="panel"><div style="font-size:1.08rem;font-weight:800;">Sobre</div><div class="muted">Versão V9.5 com design mais bonito e corporativo. Regra visual: abaixo da meta = vermelho | acima ou igual à meta = verde.</div></div>', unsafe_allow_html=True)
