...
st.set_page_config(page_title='RFT Qualidade - V7.2', page_icon='Q', layout='wide', initial_sidebar_state='expanded')
...
def existing_range_for_posto_year(conn, posto, year):
    ...
    if df.empty or pd.isna(df.loc[0, 'min_d']): return None, None
...
def render_dashboard(info, posto, ano, base_summary, periods, meta):
    ...
    with c1: st.markdown(metric_card_html('Semanal', periods['Semanal'], f"Semana atual: {periods['week_label']}"), unsafe_allow_html=True)
    with c2: st.markdown(metric_card_html('Mensal', periods['Mensal'], f"Mês atual: {periods['month_label']}"), unsafe_allow_html=True)
    with c3: st.markdown(metric_card_html('YTD', periods['YTD'], 'Acumulado do ano até hoje'), unsafe_allow_html=True)
    l,r = st.columns([1.2,1])
    with l:
        st.subheader('Resumo executivo da base')
        for label, value in [('Data mínima da base', base_summary['min_date'].strftime('%d/%m/%Y') if base_summary['min_date'] else '-'), ('Data máxima da base', base_summary['max_date'].strftime('%d/%m/%Y') if base_summary['max_date'] else '-'), ('Última data com dado', base_summary['last_date'].strftime('%d/%m/%Y') if base_summary['last_date'] else '-'), ('Dias cobertos', str(base_summary['days_covered'])), ('Uploads consolidados', str(base_summary['uploads_count'])), ('Status da base', base_summary['status'])]:
            st.markdown(kv_html(label, value), unsafe_allow_html=True)
    with r:
        st.markdown(metric_card_html('Meta x resultado (mensal)', periods['Mensal'], f"Meta atual: {str(meta).replace('.', ',')}%"), unsafe_allow_html=True)

def render_rolling12_tab(rolling12, rolling12_table):
    st.subheader('Rolling 12 Months')
    ...
    c1,c2,c3 = st.columns(3)
    with c1: st.markdown(metric_card_html('Rolling 12 Consolidado', rolling12, 'Indicador móvel de 12 meses'), unsafe_allow_html=True)
    with c2:
    ...
    if rolling12_table.empty:
        ...
        show = rolling12_table.copy()
        show['RFT'] = show['RFT'].map(lambda x: '' if pd.isna(x) else f'{x:.2f}'.replace('.', ',') + '%')
        st.dataframe(show, use_container_width=True, hide_index=True)

def render_tendencia_tab(df, ano):
    st.subheader('Tendência do RFT normal')
    st.caption('RFT normal igual à V7.0 e agora com o gráfico mensal no mesmo estilo da semanal: ambos em colunas.')
    mt = monthly_trend(df, ano)
    wt = weekly_trend(df, ano)
    c1,c2 = st.columns(2)
    with c1:
        st.markdown('<div class="section"><div class="muted">Mensal (colunas)</div></div>', unsafe_allow_html=True)
        if mt.empty:
            st.info('Sem dados mensais.')
        else:
            st.bar_chart(mt.set_index('Mes')[['RFT']], use_container_width=True)
            show_mt = mt.copy() ; show_mt['RFT'] = show_mt['RFT'].map(lambda x: f'{x:.2f}'.replace('.', ',') + '%')
            st.dataframe(show_mt, use_container_width=True, hide_index=True)
    with c2:
        st.markdown('<div class="section"><div class="muted">Semanal (colunas)</div></div>', unsafe_allow_html=True)
        if wt.empty:
            st.info('Sem dados semanais.')
        else:
            st.bar_chart(wt.set_index('Semana')[['RFT']], use_container_width=True)
            show_wt = wt.copy() ; show_wt['RFT'] = show_wt['RFT'].map(lambda x: f'{x:.2f}'.replace('.', ',') + '%')
            st.dataframe(show_wt, use_container_width=True, hide_index=True)

def render_upload_tab(conn):
...
def render_about_tab(meta):
    st.subheader('Sobre')
    st.write(f'Versão V7.2 com RFT normal mantido igual à V7.0 e sistema de Rolling 12 separado em outra aba. Gráfico mensal em colunas igual ao semanal e gráfico rolling também em colunas. Meta visual ativa: {str(meta).replace(".", ",")}%')

def main():
    conn = get_conn()
    init_db(conn)
    st.markdown("### Qualidade | RFT Automatico - V7.2")
    st.caption('RFT normal da V7.0 + aba separada para Rolling 12.')
    with st.sidebar:
    ...
    tabs = st.tabs(['Dashboard', 'Rolling 12', 'Base & Upload', 'Historico', 'Sobre'])
    ...
    with tabs[0]:
        if df.empty:
            ...
            periods = compute_weekly_monthly_ytd(df, anchor)
            render_dashboard(info, posto, ano, base_summary, periods, meta)
            render_tendencia_tab(df, ano)
    with tabs[1]:
        if df.empty:
            st.info('Sem histórico válido para compor o Rolling 12.')
        else:
            rolling12, rolling12_table = compute_roll12_from_monthly(df, df['DT_HR_INSPECAO'].dt.date.max())
            render_rolling12_tab(rolling12, rolling12_table)
    with tabs[2]:
        render_upload_tab(conn)
    with tabs[3]:
        render_history_tab(conn)
    with tabs[4]:
        render_about_tab(meta)
